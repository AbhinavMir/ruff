...
use salsa::Database;
use ty_project::metadata::ScriptSyncResult;
...
impl MainLoop {
    fn new(mode: MainLoopMode, printer: Printer) -> (Self, MainLoopCancellationToken) {
        ...
        progress.progress.set_draw_target(printer.progress_target());
    ...
    fn main_loop(mut self, db: &mut ProjectDatabase) -> Result<ExitStatus> {
        ...
        let script_sync_results = db.script_environments().sync_results();
        ...
        while let Ok(message) = crossbeam_channel::select_biased! {
            recv(script_sync_results) -> result => {
                result.map(MainLoopMessage::ScriptSyncCompleted)
            }
            ...
            match message {
                ...
                MainLoopMessage::ScriptSyncCompleted(result) => {
                    if complete_script_sync(db, result) {
                        revision += 1;
                    }
                    request_check(db, &check_sender);
                }
                ...
                MainLoopMessage::ApplyChanges(changes) => {
                    ...
                    self.schedule_resync_scripts(db);
    ...
    fn schedule_resync_scripts(&mut self, db: &mut ProjectDatabase) {
        let environments = db.script_environments().clone();
        environments.schedule_resync_all(db, &|db, file| self.progress.for_script(db, file));
    }
}
...
enum MainLoopMessage {
    ...
    ScriptSyncCompleted(ScriptSyncResult),
...
fn complete_script_sync(db: &mut ProjectDatabase, result: ScriptSyncResult) -> bool {
    let environments = db.script_environments().clone();
    !environments.complete_sync(db, result).is_empty()
}
