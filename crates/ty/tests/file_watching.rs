...
mod uv_metadata {
    ...
    fn update_and_synchronize_script(case: &mut TestCase, source: &str) -> anyhow::Result<bool> {
        ...
        if !environments.contains(file) {
            return Ok(false);
        }
        let receiver = environments.sync_results();
        let mut changed = environments.schedule_sync(&mut case.db, file, &|_, _| None);
        ...
        while environments.has_pending_synchronizations() {
            ...
            changed.extend(environments.complete_sync(&mut case.db, result));
        }
