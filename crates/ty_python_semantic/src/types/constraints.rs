...
pub(crate) enum PathBounds<'db> {
    ...
    Constrained {
        ...
        /// TypeVars constrained to `int` and `str`, the paths fix `T` to `int` and `str`
...
impl<'db> PathBounds<'db> {
    ...
    /// correlations between an inferable TypeVar and a TypeVar supplied by the caller: each path
    /// remembers only the concrete alternative selected on that path. Without this recovery, an
    /// identity-like call through two compatible constrained TypeVars returns the union of their
