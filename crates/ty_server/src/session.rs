...
use ty_project::metadata::{Options, ScriptSyncResult};
use ty_project::watch::{ChangeEvent, CreatedKind};
use ty_project::{ChangeResult, Db as _, ProjectDatabase, ProjectMetadata, SemanticDb as _, UseUv};
...
impl Session {
    ...
    /// Returns each project's background script synchronization receiver.
    pub(crate) fn script_sync_receivers(
        &self,
    ) -> Vec<(
        SystemPathBuf,
        crossbeam::channel::Receiver<ScriptSyncResult>,
    )> {
        self.projects
            .iter()
            .map(|(root, state)| (root.clone(), state.db.script_environments().sync_results()))
            .collect()
    }

    /// Synchronizes a script, creating its environment entry if necessary.
    pub(crate) fn synchronize_script(&mut self, client: &Client, path: &AnySystemPath) {
        ...
        let changed_paths = Self::schedule_script_sync(db, file, client, capabilities);

        self.script_environments_changed(client, changed_paths);
    }

    /// Synchronizes every project that previously discovered an environment for this script.
    pub(crate) fn synchronize_known_script(&mut self, client: &Client, path: &SystemPath) {
        let capabilities = self.resolved_client_capabilities;
        ...
        for state in self.projects.values_mut() {
            ...
            // Open documents are synchronized when opened or saved. Ignore watcher events for
            // them because uv reads the file on disk, which may differ from the open document.
            // Closed files only need refreshing if this project already initialized a script
            // environment for them.
            if db.is_open_file(file) || !db.script_environments().contains(file) {
                continue;
            }
            changed_paths.extend(Self::schedule_script_sync(db, file, client, capabilities));
        }

        self.script_environments_changed(client, changed_paths);
    }

    /// Applies one result from background script synchronization.
    pub(crate) fn complete_script_sync(
        &mut self,
        client: &Client,
        project_root: &SystemPath,
        result: ScriptSyncResult,
    ) {
        let result_path = result.path().to_path_buf();
        let changed_paths = {
            let Some(project) = self.projects.get_mut(project_root) else {
                tracing::debug!(
                    "Discarded script synchronization for removed project `{project_root}` and script `{}`",
                    result_path
                );
                return;
            };
            ...
            let changed_files = environments.complete_sync(db, result);
            Self::script_paths(db, changed_files)
        };
    ...
    fn schedule_script_sync(
        ...
        let environments = db.script_environments().clone();
        let changed_files = environments.schedule_sync(db, file, &|db, file| {
    ...
    fn script_paths(db: &dyn Db, files: Vec<File>) -> Vec<SystemPathBuf> {
        files
            .into_iter()
