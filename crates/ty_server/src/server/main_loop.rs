...
impl Server {
        ...
        while let Ok(next_event) = self.next_event() {
            ...
            match next_event {
                ...
                Event::ScriptSync {
                    ...
                    self.session
                        .complete_script_sync(&client, &project_root, result);
                }
            }
        }
    ...
    fn next_event(&mut self) -> Result<Option<Event>, crossbeam::channel::RecvError> {
        ...
        let script_sync = ScriptSyncReceivers(self.session.script_sync_receivers());
...
/// The result receivers for the currently active project databases.
struct ScriptSyncReceivers(
    Vec<(
        SystemPathBuf,
...
impl ScriptSyncReceivers {
    /// Waits for a project result, client message, or main-loop action.
    fn select(
        &self,
        ...
        if let Some((project_root, receiver)) = self.0.get(index) {
            return operation.recv(receiver).map(|result| {
                Some(Event::ScriptSync {
