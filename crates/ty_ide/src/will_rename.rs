...
use ty_python_semantic::{
    HasType, ImportAliasResolution, ResolvedDefinition, SemanticModel, ValueProviders,
    binding_type, definitions_for_imported_symbol,
};
...
impl<'db> SemanticPass<'_, 'db> {
    fn name(&mut self, name: &ast::ExprName) {
        ...
        let decision = self.model.name_load(name).map_or_else(
            ...
            |load| {
                let decision = Self::summarize_providers(load.providers(), |definition| {
                ...
                .decision(affected);
                if load.crosses_scope_declaration() && matches!(decision, Decision::Replace(_)) {
                    Decision::Unsupported
                } else {
                    decision
                }
    ...
    fn summarize_providers(
        ...
        for definition in providers.definitions() {
            summary.record_definition(definition, &replacement_for);
        }
        if providers.has_unrepresented_provider() || providers.may_be_deleted() {
            summary.issue = ProviderIssue::Unknown;
        }
        summary.can_fall_through = !providers.is_definitely_bound();
        summary
    }
    ...
    fn attribute(&mut self, attribute: &ast::ExprAttribute) -> TraversalSignal {
        ...
        let providers = receiver_module.and_then(|module| {
            self.model
                .module_global_providers(module, attribute.attr.as_str())
        });
        let has_providers = providers.is_some();
        let mut root = &*attribute.value;
        while let ast::Expr::Attribute(attribute) = root {
            root = &attribute.value;
        }
        let decision = match providers {
            Some(providers) => {
                let summary = Self::summarize_providers(&providers, |definition| {
                    if rewrite.is_some()
        ...
        let unresolved_file = !has_providers
            && receiver_module
                .and_then(|module| {
    ...
    fn is_implicitly_imported_submodule(&self, root: &ast::Expr, module: Module<'_>) -> bool {
        ...
        let summary = Self::summarize_providers(load.providers(), |_| Ok(None));
